...
def create_monster():
    ...
    if side == 'top':
        return [random.randint(0, width), -monster_size // 2, 0, 0]
    elif side == 'bottom':
        return [random.randint(0, width), height + monster_size // 2, 0, 0]
    elif side == 'left':
        return [-monster_size // 2, random.randint(0, height), 0, 0]
    else:  # right
        return [width + monster_size // 2, random.randint(0, height), 0, 0]
...
def draw_monster(screen, x, y, size):
    pygame.draw.rect(screen, RED, (x - size // 2, y - size // 2, size, size))
    # Draw eyes
    eye_size = size // 5
