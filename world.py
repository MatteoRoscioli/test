...
def use_ability():
    global player_speed, player_radius, ability_duration, ability_cooldown, lives, player_x, player_y
    ...
    if game_state == PLAYING:
        ...
        for coin in current_room['coins']:
            ...
            if player_rect.colliderect(coin_rect):
                ...
                collected_coins += 1
            elif skins[current_skin]['ability'] == 'coin_magnet' and ability_duration > 0:
                # Coin magnet ability: Attract nearby coins
                distance = ((coin['x'] - player_x) ** 2 + (coin['y'] - player_y) ** 2) ** 0.5
                if distance < 100:  # Magnet range
                    coin['x'] += (player_x - coin['x']) * 0.1  # Move coin towards player
                    coin['y'] += (player_y - coin['y']) * 0.1
                    if distance < player_radius:  # Collect coin if it's very close
                        coins_to_remove.append(coin)
                        collected_coins += 1
