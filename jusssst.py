...
pygame.display.set_caption("Harry the Great")
# Wizard properties
wizard_x = width // 2
wizard_y = height // 2
wizard_speed = 7
...
def draw_wizard(screen, x, y, size):
    # Draw body (changed from BLUE to BLACK)
    pygame.draw.rect(screen, BLACK, (x - size // 4, y, size // 2, size // 2))
    # Draw head
    pygame.draw.circle(screen, SKIN, (x, y - size // 4), size // 4)
    # Draw hat (changed from PURPLE to BLACK)
    pygame.draw.polygon(screen, BLACK, [(x - size // 3, y - size // 4),
                                        (x + size // 3, y - size // 4),
                                        (x, y - size)])
